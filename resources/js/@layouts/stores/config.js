...
export const useLayoutConfigStore = defineStore('layoutConfig', () => {
  ...
  // Add computed property to check if we're on a board page
  const isBoardPage = computed(() => {
    // Check if the route path matches any of the board page patterns
    return route.path.includes('/boards/') || 
           (route.path.includes('/projects/') && route.path.includes('/boards/')) ||
           (route.path.includes('/organisations/') && route.path.includes('/projects/') && route.path.includes('/boards/'))
  })
  ...
  const _layoutClasses = computed(() => {
    ...
    return [
      ...
      // Only add the layout-content-width class if we're not on a board page
      { [`layout-content-width-${appContentWidth.value}`]: !isBoardPage.value },
